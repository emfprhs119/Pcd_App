...
public class DrawPcd extends View {
	...
	void drawSet(Canvas canvas, Iterator<Double> it, Arrow arrow) {
		...
		while (it.hasNext()) {
			...
			if (isRight) {
				...
				canvas.drawText(String.format("%.2f", Math.abs(size) / rad * pcd_rad), (float) (centerX + diff),
						(float) (centerY + (Math.abs(size) / 2 * upDown)), paint);
				...
				canvas.drawLine(centerX + diff,
						(float) (centerY + ((size + (upDown < 0 ? diff_turm : diff_turm / 2)) * upDown) / 2),
						centerX + diff, (float) (centerY + size * upDown), paint);
				// assist line
				canvas.drawLine(centerX + diff, (float) (centerY + size * upDown), centerX,
						(float) (centerY + size * upDown), paint);
				canvas.drawLine(centerX + diff, centerY, centerX, centerY, paint);
				diff += diff_turm * 1.5;
			} else {
				canvas.drawText(String.format("%.2f", Math.abs(size) / rad * pcd_rad),
						(float) (centerX + Math.abs(size) / 2),
	...
	public void angleCalc(double pcd, int deg) {
		...
		pointArr[0] = new Point(heightd, widthd);
	}
